...
import com.agri.entity.CropDashboard;
import com.agri.entity.CropDashboardModel;

public interface CropDashboardRepository extends JpaRepository<CropDashboard, Integer> {
	...
	@Query(value = "select  c.fasal_id,c.fasal_type_id,c.f_description, " +
			"sum(c.total_agri_area) as total_agri_area,sum(c.gata_count) as gata_count " +
					...
					" where c.district_code_census =:districtCode and c.tehsil_code_census =:tehsilCode and c.village_code_census=:villageCode  group by " +
	...
	@Query(value = "select  c.fasal_id,c.fasal_type_id,c.f_description, " +
			"sum(c.total_agri_area) as total_agri_area,sum(c.gata_count) as gata_count " +
					"from cropdashboard_1431 c  " +
					" where c.district_code_census =:districtCode and c.tehsil_code_census =:tehsilCode group by " +
	...
	@Query(value = "select  c.fasal_id,c.fasal_type_id,c.f_description, " +
			"sum(c.total_agri_area) as total_agri_area,sum(c.gata_count) as gata_count " +
					"from cropdashboard_1431 c  " +
					" where c.district_code_census =:districtCode  group by " +
			" c.fasal_id,c.fasal_type_id,c.f_description order by total_agri_area desc ", nativeQuery = true)
	...
	List<CropDashboardModel> getCropByAllDist1431(@Param("fasal_type_id") String fasal_type_id);
	...
	"inner join up_tehsil_master t on a.tehsil_code_census=t.tehsil_code_census " +
			"inner join up_district_master d on d.district_code_census = a.district_code_census  order by total_agri_area desc", nativeQuery = true)
	List<CropDashboardModel> getCropByTeh1431( @Param("districtCode") String districtCode,  @Param("fasal_type_id") String fasal_type_id);
